...
export default function EditTspAccountDialog({
  open,
  handleClose,
  endpoint,
  setAccountsRefresh,
  ...
  const handleContributionChange = (event) => setContribution(event.target.value);
  const handleGovtContributionChange = (event) => setGovtContribution(event.target.value);
  ...
  const handleSubmit = () => {
    patchAccount(accountHolder, accountType, value, totalReturn, contribution, govtContribution, ytdReturnPercentage,  row.id).then(() => {
      handleModalClose();
    });
  };

  function patchAccount(accountHolder, accountType, value, totalReturn, contribution, govtContribution, ytdReturnPercentage, id) {
    let payload = { accountHolder, accountType, value, totalReturn, contribution, govtContribution, ytdReturnPercentage, id };
  ...
  useEffect(() => {
    ...
    setGovtContribution(row.govtContribution)
          ...
          <Grid container spacing={2}>
            ...
            <Grid item xs={4}>
              <TextField
                ...
                value={contribution}
                placeholder="Enter the your contribution as a percent"
              />
            </Grid>
            <Grid item xs={4}>
              <TextField
                id="outlined-basic"
                label="GovtContribution"
                variant="outlined"
                onChange={handleGovtContributionChange}
                value={govtContribution}
                placeholder="Enter the the Government's contribution as a percent"
              />
            </Grid>
            <Grid item xs={4}>
              <TextField
